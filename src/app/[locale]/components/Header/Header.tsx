...
export const Header = ({ translations, locale }: HeaderProps) => {
  const [activeLink, setActiveLink] = useState(`/home`);
  const navLinks = [
    { path: "home", label: translations.home },
    { path: "about", label: translations.about },
    { path: "experiences", label: translations.experiences },
    { path: "contacts", label: translations.contacts },
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (aboutOffset && scrollPosition < aboutOffset - 60) {
        return setActiveLink("/home");
      } else if (experiencesOffset && scrollPosition < experiencesOffset - 60) {
        return setActiveLink("/about");
      } else if (contactOffset && scrollPosition < contactOffset - 60) {
        return setActiveLink("/experiences");
      } else if (blogOffset && scrollPosition < blogOffset - 60) {
        return setActiveLink("/contacts");
      } else {
        return setActiveLink("/blog");
      }
    };
